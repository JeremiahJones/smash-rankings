namespace :populate_players_and_rankings do
  ...
  task :all => [:rankings_2017, :rankings_2016, :rankings_2015, :rankings_2014, :rankings_2013] do
    puts "\rFinished populating all players and rakings"
  end
  ...
  task rankings_2017: :environment do
    ...
    rows.map do |row|
      ...
      @player = Player.find_or_create_by( gamer_tag: row_values[2],
                               country: row.css('td')[1].css('a')[0]['title'],
                               characters: row.css('td')[3].css('img').map{ |i| i['alt']}
      )
      @player.miom_ranks.create(year: '2017',
                                score: row_values[4].to_i,
                                change: (row_values[5].squish.include?('9000') ?  'NEW' : row_values[5].squish ),
                                rank: row_values[0].to_i
      )
    end
  ...
  task rankings_2016: :environment do
    ...
    rows.map do |row|
      ...
      @player.miom_ranks.create(year: '2016',
                                score: row_values[4].to_i,
                                change: (row_values[5].squish.include?('9000') ?  'NEW' : row_values[5].squish ),
                                rank: row_values[0].to_i
      )
  ...
  task rankings_2015: :environment do
    ...
    rows.map do |row|
      ...
      @player.miom_ranks.create(year: '2015',
                                score: row_values[4].to_i,
                                change: (row_values[5].squish.include?('9000') ?  'NEW' : row_values[5].squish ),
                                rank: row_values[0].to_i
      )
  ...
  task rankings_2014: :environment do
    ...
    rows.map do |row|
      ...
      @player.miom_ranks.create(year: '2014',
                                score: row_values[4].to_i,
                                change: (row_values[5].squish.include?('9000') ?  'NEW' : row_values[5].squish ),
                                rank: row_values[0].to_i
      )
  ...
  task rankings_2013: :environment do
    ...
    rows.map do |row|
      ...
      @player.miom_ranks.create(year: '2013',
                                score: row_values[4].to_i,
                                change: (row_values[5].squish.include?('9000') ?  'NEW' : row_values[5].squish ),
                                rank: row_values[0].to_i
      )
